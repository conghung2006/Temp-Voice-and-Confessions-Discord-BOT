...
confession_channel_id = int(os.getenv('CFS_ID'))
confession_create_channel_id = int(os.getenv('CFS_CRT_ID'))
...
async def is_true_channel(ctx):
	if ctx.channel.id == confession_create_channel_id:
		...
		temp = ctx.send(embed = disnake.Embed(description=f"**Bạn chỉ có thể tạo confessions ở kênh {crt_channel} .**",color=0xff0000))
		return False
